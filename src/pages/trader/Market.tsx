  ...
  return (
    <div className="min-h-screen overflow-y-auto w-full">
      <ToastContainer
        ...
        theme="colored"
      />
      <nav className="grid grid-cols-9 z-20 sticky top-0  place-content-center w-full place-items-center py-3 dark:bg-[#09090b] border-b-2 border-violet-300 dark:border-[#262626]">
      ...
      <div className="body px-5 max-h-40 w-full grid grid-cols-12 grid-flow-row">
        ...
        <div className={`items-category relative  grid  gap-3 p-5 col-span-9 lg:col-span-10 rounded-xl dark:dark:bg-zinc-900  ${isItemSliderOpen ? '' : 'overflow-hidden'} ${byCategory?.length === 0 ? 'grid-cols-1' : 'grid-cols-3 lg:grid-cols-4'}`}>
              <div className={`organizations  bg-gray-200 rounded-xl dark:bg-zinc-800 w-full min-h-full absolute top-0 left-0 z-10 ${ isItemSliderOpen ? 'translate-x-0' : 'translate-x-full'} transition-all duration-500`}>
                ...
                <div className="item px-3 pb-4">
                  ...
                  <h3 className="text-xl dark:text-gray-300 py-3 font-pop font-semibold capitalize">{gettedItemData?.name}</h3>
                  <h6 className="text dark:text-gray-300 opacity-80 font-pop font-semibold capitalize">₱{gettedItemData?.price}</h6>
                  <h6 className="text-sm dark:text-gray-300 opacity-80  capitalize">Listed in {gettedItemData?.location}</h6>
                  <div className="details dark:text-gray-100">
                    ...
                    <Link to={`/trader/${gettedItemData?.user.username}`} className="py-3 mt-6 font-extrabold uppercase text-center bg-indigo-700 w-full block rounded-lg">Contact Trader</Link>
                  </div>
                </div>
            </div>
            {byCategory?.length === 0
